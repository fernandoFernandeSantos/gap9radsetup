...
typedef struct testcase {
    ...
    void (*test)(testresult_t *r, void(*start)(), void(*stop)());
} testcase_t;


void run_suite_start() {
...
int run_suite(testcase_t *tcs) {
    int error_count = 0;
    while (tcs->name) {
        printf("test [%s] starts\n", tcs->name);
        testresult_t r = {0};
        tcs->test(&r, run_suite_start, run_suite_stop);
        printf("test [%s] ends, status = %d\n", tcs->name, r.errors);
...
/////////////////////////////////////////////////////////
int main_test() {
    int errors = run_suite(testcases);
    if (!errors) {
