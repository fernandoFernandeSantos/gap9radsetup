...
//    printf("Entering main controller\n");
    ...
    image_in = (uint16_t *) pi_l2_malloc(size_img_in);
    if (image_in == NULL) {
    ...
    for (uint32_t i = 0; i < 3; i++) {
        Out_Layer[i] = (int16_t *) pi_l2_malloc(Out_Layer_Size[i]);
        if (Out_Layer[i] == NULL) {
    ...
    Mnist_L1_Memory = pi_l1_malloc(&cluster_dev, _Mnist_L1_Memory_SIZE);
...
//        }
    ...
    pi_l1_free(&cluster_dev, Mnist_L1_Memory, _Mnist_L1_Memory_SIZE);
