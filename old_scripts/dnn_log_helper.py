...
def start_setup_log_file(device: str, args_conf: list, dnn_name: str, activate_logging: bool, dnn_goal: str) -> None:
    global __LOGGING_ACTIVE
    __LOGGING_ACTIVE = activate_logging
    dnn_log_header = f"device:{device} dnn_goal:{dnn_goal} " + " ".join(args_conf)
    if __LOGGING_ACTIVE:
        bench_name = f"{device}-{dnn_name}"
