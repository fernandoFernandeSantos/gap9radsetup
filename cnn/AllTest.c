...
#define SETUP_RADIATION_ITERATIONS 16384

#elif RAD_CNN_OP == RAD_SEQUENTIAL_LINEAR || RAD_CNN_OP == RAD_PARALLEL_VECT_LINEAR
#include "inputs_linear.h"
//This was set in the last hours of 27/09/2022
#define SETUP_RADIATION_ITERATIONS 131072
...
#define gap_resethwtimer()                     pulp_write32(ARCHI_FC_TIMER_ADDR + PLP_TIMER_CFG_REG_LO, plp_timer_conf_get(1,1,0,0,0,0,0,0,0))
#define gap_cl_readhwtimer()                      pulp_read32(ARCHI_FC_TIMER_ADDR + PLP_TIMER_VALUE_LO)
...
#ifndef RISCV
#define B_ins(dst, src, size, off)      gap_bitinsert(dst, src, size, off)
#define B_ins_r(dst, src, size, off)    gap_bitinsert_r(dst, src, size, off)
#define B_ext(x, size, off)             gap_bitextract(x, size, off)
#define B_extu(x, size, off)            gap_bitextractu(x, size, off)
#define B_ext_r(x, size, off)           gap_bitextract_r(x, size, off)
#define B_extu_r(x, size, off)          gap_bitextractu_r(x, size, off)
...

{
    ...
    for (int c=0; c<Wo; c++)
        for (int l=0; l<Ho; l++) {
            ...
            Out[l*Wo+c] = gap_dotp4(V0, ((v4s){1,1,1,1}))>>2;
        }
}
...

{
    for (int i=0; i<H; i++) {
        ...
        v4s *I = (v4s *)In;
        for (int j=0; j<(W>>3); j++) {
            R = gap_sumdotp4(I[2*j], F[2*j], R);
            R = gap_sumdotp4(I[2*j+1], F[2*j+1], R);
        }
        // if ((W&0x3)!=0) R = gap_sumdotp4(I[W>>2]&LinearMask[W&0x3], F[W>>2], R);
...

{
	...
	for (int c=0; c<Wo; c++)
		for (int l=0; l<Ho; l++) {
			v2s V0 = *((v2s *)(In+2*l*W+2*c)), V1 = *((v2s *)(In+(2*l+1)*W+2*c));
			int R=gap_dotp2(V0, ((v2s){1,1}));
			Out[l*Wo+c] = gap_sumdotp2(V0, ((v2s){1,1}), R)>>2;
		}
}
...

{
        for (int i=0; i<H; i++) {
                ...
                v2s *I = (v2s *)In;
                for (int j=0; j<(W>>2); j++) {
                        R = gap_sumdotp2(I[2*j], F[2*j], R);
                        R = gap_sumdotp2(I[2*j+1], F[2*j+1], R);
                }
...
//			for (int i=0; i<Iter; i++)
        ...
        case 9:
            ...
            WriteGpio(GPIO, 1);
            Ti = gap_cl_readhwtimer();
            for (int i=0; i<Iter; i++) pi_cl_team_fork(8, (void *) ParMaxPooling, (void *) &Arg);
            Ti = gap_cl_readhwtimer() - Ti;
            WriteGpio(GPIO, 0);
            *num_ops = Ti;
            if (Trace) printf("[%2d][%s] %7d %35s: %8d cycles, %1d Cores\n", Which, Mode, (Wi/2)*(Hi/2)*Iter, "2x2/2 Parallel Max Pool", Ti, gap_ncore());
            break;
        case 10:
            IN = Mem;
            OUT = Mem + Wi * Hi;
            ...
            WriteGpio(GPIO, 1);
            Ti = gap_cl_readhwtimer();
            for (int i = 0; i < Iter; i++) pi_cl_team_fork(gap_ncore(), (void *) ParAvgPooling, (void *) &Arg);
            Ti = gap_cl_readhwtimer() - Ti;
            WriteGpio(GPIO, 0);
            *num_ops = Ti;
            if (Trace)
                printf("[%2d][%s] %7d %35s: %8d cycles, %1d Cores\n", Which, Mode, (Wi / 2) * (Hi / 2) * Iter,
                       "2x2/2 Parallel Avg Pool", Ti, gap_ncore());
            break;
        case 11:
            IN = Mem; OUT = Mem+Wic*Hic; FILTER = Mem+Wic*Hic+(Wic-4)*(Hic-4); CheckMem(Wic*Hic+(Wic-4)*(Hic-4)+5*5);
            ...
            WriteGpio(GPIO, 1);
            Ti = gap_cl_readhwtimer();
            for (int i=0; i<Iter; i++) pi_cl_team_fork(gap_ncore(), (void *) ParAdditive5x5Convolution, (void *) &Arg);
            Ti = gap_cl_readhwtimer() - Ti;
            WriteGpio(GPIO, 0);
            *num_ops = Ti;
            if (Trace) printf("[%2d][%s] %7d %35s: %8d cycles, %1d Cores\n", Which, Mode, (Wic-4)*(Hic-4)*Iter, "Parallel 5x5 Convolution", Ti, gap_ncore());
            break;
        case 12:
            IN = Mem; OUT = Mem+Wil; FILTER = Mem+Wil+Hil; CheckMem(Wil+Hil + Wil*Hil);
            ...
            WriteGpio(GPIO, 1);
            Ti = gap_cl_readhwtimer();
            for (int i=0; i<Iter; i++) pi_cl_team_fork(gap_ncore(), (void *) ParLinear, (void *) &Arg);
            Ti = gap_cl_readhwtimer() - Ti;
            WriteGpio(GPIO, 0);
            *num_ops = Ti;
            if (Trace) printf("[%2d][%s] %7d %35s: %8d cycles, %1d Cores\n", Which, Mode, Wil*Hil*Iter, "Parallel Linear", Ti, gap_ncore());
            break;
        case 13:
        {
            unsigned int In = (unsigned int) Mem;
            ...
            WriteGpio(GPIO, 1);
            Ti = gap_cl_readhwtimer();
            for (int i=0; i<Iter; i++) pi_cl_team_fork(gap_ncore(), (void *) ParXnorConv5x5, (void *) &Arg1);
            Ti = gap_cl_readhwtimer() - Ti;
            WriteGpio(GPIO, 0);
            *num_ops = Ti;
        }
            if (Trace) printf("[%2d][%s] %7d %35s: %8d cycles, %1d Cores\n", Which, Mode, (Wxor-4)*(Hxor-4)*Iter, "Parallel Xnor Conv 5x5", Ti, gap_ncore());
            break;
        case 14:
            IN = Mem;
            OUT = Mem + Wi * Hi;
            ...
            Ti = gap_cl_readhwtimer();
//            for (int i = 0; i < Iter; i++)
            pi_cl_team_fork(gap_ncore(), (void *) ParMaxPoolingVect, (void *) &Arg);
            Ti = gap_cl_readhwtimer() - Ti;
            WriteGpio(GPIO, 0);
            *num_ops = Ti;
            if (Trace)
                printf("[%2d][%s] %7d %35s: %8d cycles, %1d Cores\n", Which, Mode, (Wi / 2) * (Hi / 2) * Iter,
                       "2x2/2 Parallel Max Pool Vect", Ti, gap_ncore());
            break;
        case 15:
            IN = Mem;
            OUT = Mem + Wi * Hi;
            ...
            WriteGpio(GPIO, 1);
            Ti = gap_cl_readhwtimer();
            for (int i = 0; i < Iter; i++) pi_cl_team_fork(gap_ncore(), (void *) ParAvgPoolingVect, (void *) &Arg);
            Ti = gap_cl_readhwtimer() - Ti;
            WriteGpio(GPIO, 0);
            *num_ops = Ti;
            if (Trace)
                printf("[%2d][%s] %7d %35s: %8d cycles, %1d Cores\n", Which, Mode, (Wi / 2) * (Hi / 2) * Iter,
                       "2x2/2 Parallel Avg Pool Vect", Ti, gap_ncore());
            break;
        case 16:
            IN = Mem;
            OUT = Mem + Wic * Hic;
...
//            CheckMem(Wic * Hic + (Wic - 4) * (Hic - 4) + 5 * 5);
            ...
            Ti = gap_cl_readhwtimer();
//            for (int i = 0; i < Iter; i++)
            pi_cl_team_fork(gap_ncore(), (void *) ParAdditive5x5ConvolutionVect, (void *) &Arg);
            Ti = gap_cl_readhwtimer() - Ti;
            WriteGpio(GPIO, 0);
            *num_ops = Ti;
            if (Trace)
                printf("[%2d][%s] %7d %35s: %8d cycles, %1d Cores\n", Which, Mode, (Wic - 4) * (Hic - 4) * Iter,
                       "Parallel Convolution Vect", Ti, gap_ncore());
            break;
        case 17:
            // IN = Mem; OUT = Mem+Wil; FILTER = Mem+Wil+Hil; CheckMem(Wil+Hil + Wil*Hil);
            IN = Mem;
            ...
            Ti = gap_cl_readhwtimer();
//            for (int i = 0; i < Iter; i++)
            pi_cl_team_fork(gap_ncore(), (void *) ParLinearVect, (void *) &Arg);
            Ti = gap_cl_readhwtimer() - Ti;
            WriteGpio(GPIO, 0);
            *num_ops = Ti;
            if (Trace)
                printf("[%2d][%s] %7d %35s: %8d cycles, %1d Cores\n", Which, Mode, Wil * Hil * Iter,
                       "Parallel Linear Vect", Ti, gap_ncore());
            break;
#endif
    }
}
...
//#endif
    // RADIATION TEST EDIT
//    PMU_set_voltage(RAD_VOLT_SET, 0);
...
//    rt_freq_set(RT_FREQ_DOMAIN_FC, RAD_FREQ_SET_FC);
//    rt_freq_set(RT_FREQ_DOMAIN_CL, RAD_FREQ_SET_CL);

    // From mobilenet gap9 nn menu
...
//    struct pi_cluster_task *task = (struct pi_cluster_task *) pi_l2_malloc(sizeof(struct pi_cluster_task));
    ...
    pi_cluster_task(&task, (void (*)(void *)) &benchmarks, &Arg);
...
//        rt_cluster_call(NULL, CID, (void *) benchmarks, &Arg, NULL, 0, 0, 8, NULL);
        ...
        cycles = pi_perf_read(PI_PERF_CYCLES) - cycles;
        end_time = pi_time_get_us();
...
//        }
    ...
    printf("RATIT:%d ITS:%d TIME_IT:%ld CYCLE_IT:%ld ACCTIME:%ld ACCCYCLES:%ld\n",
           SETUP_RADIATION_ITERATIONS, its, tot_time, cycles, acc_time, acc_ops);
